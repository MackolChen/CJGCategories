...
#import "CLLocation+CJGCH1903.h"


@implementation CLLocation (CJGCH1903)


- (id) initWithCH1903x:(double)x y:(double)y
{
    self = [self initWithLatitude:[CLLocation cjg_CHtoWGSlatWithX:x y:y] longitude:[CLLocation cjg_CHtoWGSlongWithX:x y:y]];
    if (self != nil) {
...
- (double)cjg_CH1903Y {
    return [CLLocation cjg_WGStoCHyWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}

- (double)cjg_CH1903X {
    return [CLLocation cjg_WGStoCHxWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}
...
+ (double)cjg_WGStoCHyWithLatitude:(double)lat longitude:(double)lng {
    ...
    lat = [CLLocation cjg_decToSex:lat];
    lng = [CLLocation cjg_decToSex:lng];
    
    // Converts degrees to seconds (sex)
    lat = [CLLocation cjg_degToSec:lat];
    lng = [CLLocation cjg_degToSec:lng];
