...
+ (void)initialize
{
    _cjg_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                       context:nil
...
- (void)cjg_faceAwareFill {
    // Safe check!
    if (self.image == nil) {
        return;
    }
    
    CGRect facesRect = [self cjg_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self cjg_scaleImageFocusingOnRect:facesRect];
}

- (CGRect) cjg_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _cjg_faceDetector;
...
- (void) cjg_scaleImageFocusingOnRect:(CGRect) facesRect {
