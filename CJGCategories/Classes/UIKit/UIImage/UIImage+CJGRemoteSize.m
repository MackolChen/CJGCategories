...
- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData *)data {
    ...
    if(!self.cjg_sizeRequestType ) {
        if( memcmp(pngSignature, cString, 8) == 0 ) {
            self.cjg_sizeRequestType = @"PNG";
        }
        else if( memcmp(bmpSignature, cString, 2) == 0 ) {
            self.cjg_sizeRequestType = @"BMP";
        }
        else if( memcmp(jpgSignature, cString, 2) == 0 ) {
            self.cjg_sizeRequestType = @"JPG";
        }
        else if( memcmp(gifSignature, cString, 2) == 0 ) {
            self.cjg_sizeRequestType = @"GIF";
        }
    }
    
    if( [self.cjg_sizeRequestType isEqualToString: @"PNG"] ) {
        char type[5];
        int offset = 8;
        ...
        if( strcmp(type, "IHDR") == 0 ) {   //Should always be first
            ...
            if( self.cjg_sizeRequestCompletion ) {
                self.cjg_sizeRequestCompletion(self, CGSizeMake(width, height));
            }
            
            self.cjg_sizeRequestCompletion = nil;
            
            [connection cancel];
        }
    }
    else if( [self.cjg_sizeRequestType isEqualToString: @"BMP"] ) {
        int offset = 18;
        dword width = 0, height = 0;
        memcpy(&width, cString+offset, 4);
        offset += 4;
    ...
    else if( [self.cjg_sizeRequestType isEqualToString: @"JPG"] ) {
        int offset = 4;
    ...
    else if( [self.cjg_sizeRequestType isEqualToString: @"GIF"] ) {
        int offset = 6;
        dword width = 0, height = 0;
...
-(void)connection:(NSURLConnection*)connection didFailWithError:(NSError *)error {
    if( self.cjg_sizeRequestCompletion )
        self.cjg_sizeRequestCompletion(self, CGSizeZero);
}
...
- (void)connectionDidFinishLoading: (NSURLConnection *)connection {
    ...
    if(!self.cjg_sizeRequestData.length) {
        self.cjg_sizeRequestData = nil;
    }
    else {
        //Try parse to UIImage
        UIImage* image = [UIImage imageWithData: self.cjg_sizeRequestData];
        
        if( self.cjg_sizeRequestCompletion && image) {
            self.cjg_sizeRequestCompletion(self, [image size]);
            return;
        }
    }
...
+ (void)requestSizeNoHeader:(NSURL*)imgURL completion:(CJGUIImageSizeRequestCompleted)completion{
    ...
    if([imgURL isFileURL] ) {
        ...
        imgURL.cjg_sizeRequestCompletion = completion;
