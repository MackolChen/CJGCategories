...
+ (UIImage *)cjg_iconWithFont:(UIFont *)font named:(NSString *)iconNamed withTintColor:(UIColor *)tintColor clipToBounds:(BOOL)clipToBounds forSize:(CGFloat)fontSize{
    ...
    UIImage *image = [[self cjg_cache] objectForKey:identifier];
    if(image == nil){
        ...
        if(!CGSizeEqualToSize(CGSizeZero, imageSize)){
            ...
            [[self cjg_cache] setObject:image forKey:identifier];
        }
    }
    return image;
}

+ (UIImage *)cjg_imageWithPDFNamed:(NSString *)pdfNamed forHeight:(CGFloat)height{
    return [self cjg_imageWithPDFNamed:pdfNamed withTintColor:nil forHeight:height];
}

+ (UIImage *)cjg_imageWithPDFNamed:(NSString *)pdfNamed withTintColor:(UIColor *)tintColor forHeight:(CGFloat)height{
    NSString *pdfFile = [[NSBundle mainBundle] pathForResource:pdfNamed ofType:@"pdf"];
    return [self cjg_imageWithPDFFile:pdfFile withTintColor:tintColor forSize:CGSizeMake(MAXFLOAT, height)];
}

+ (UIImage *)cjg_imageWithPDFFile:(NSString *)pdfFile withTintColor:(UIColor *)tintColor forSize:(CGSize)size{
