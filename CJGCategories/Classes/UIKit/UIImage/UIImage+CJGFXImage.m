...
- (UIImage *)cjg_imageScaledToFitSize:(CGSize)size
{
    ...
    {
        return [self cjg_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self cjg_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}
...
- (UIImage *)cjg_reflectedImageWithScale:(CGFloat)scale
{
	...
	//clip to gradient
	CGContextClipToMask(context, bounds, [[self class] cjg_gradientMask]);
...
{
    //get reflected image
    UIImage *reflection = [self cjg_reflectedImageWithScale:scale];
