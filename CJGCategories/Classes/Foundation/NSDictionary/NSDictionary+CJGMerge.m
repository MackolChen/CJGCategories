...
@implementation NSDictionary (CJGMerge)
/**
    ...
    [resultTemp enumerateKeysAndObjectsUsingBlock: ^(id key, id obj, BOOL *stop) {
        if ([dict1 objectForKey:key]) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                NSDictionary * newVal = [[dict1 objectForKey: key] cjg_dictionaryByMergingWith: (NSDictionary *) obj];
        ...
        {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                NSDictionary * newVal = [[dict2 objectForKey: key] cjg_dictionaryByMergingWith: (NSDictionary *) obj];
                [result setObject: newVal forKey: key];
            } else {
                [result setObject: obj forKey: key];
            }
...
- (NSDictionary *)cjg_dictionaryByMergingWith:(NSDictionary *)dict {
    return [[self class] cjg_dictionaryByMerging:self with: dict];
}
