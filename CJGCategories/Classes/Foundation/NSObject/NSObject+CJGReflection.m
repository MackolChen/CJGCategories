...
{
    return [[self class] cjg_propertyKeys];
}
+ (NSArray *)cjg_propertyKeys {
...
{
    return [[self class] cjg_propertiesInfo];
}
/**
...
+ (NSArray *)cjg_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self cjg_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)cjg_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] cjg_propertiesInfo];
...
//            NSString *argString = [NSString stringWithCString:arg encoding:NSUTF8StringEncoding];
            [arguments addObject:[[self class] cjg_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] cjg_decodeType:returnType];
        NSString *encodeString = [[self class] cjg_decodeType:encoding];
...
-(NSDictionary *)cjg_protocolList{
    return [[self class]cjg_protocolList];
}
+ (NSDictionary *)cjg_protocolList
{
...
+ (NSArray *)cjg_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] cjg_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)cjg_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)cjg_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString cjg_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
