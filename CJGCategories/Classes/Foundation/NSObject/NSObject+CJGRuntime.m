...
BOOL cjg_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void cjg_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void cjg_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
{
    cjg_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)cjg_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    cjg_method_append(self.class, klass, newMethod);
}

+ (void)cjg_replaceMethod:(SEL)method fromClass:(Class)klass
{
    cjg_method_replace(self.class, klass, method);
}

- (BOOL)cjg_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class cjg_instancesRespondToSelector:selector untilClass:stopClass];
}

- (BOOL)cjg_superRespondsToSelector:(SEL)selector
{
    return [self.superclass instancesRespondToSelector:selector];
}

- (BOOL)cjg_superRespondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.superclass cjg_instancesRespondToSelector:selector untilClass:stopClass];
}

+ (BOOL)cjg_instancesRespondToSelector:(SEL)selector untilClass:(Class)stopClass
{
