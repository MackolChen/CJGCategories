...
+ (id)cjg_doInstanceMethodTarget:(id)target selectorName:(NSString *)selectorName args:(NSArray *)args
{
    ...
    [invocation cjg_setArgumentsWithArray:args];
...
- (void)cjg_setArgumentsWithArray:(NSArray *)array
{
    if (nil != array && array.count > 0) {
        ...
        if (array.count != numargs) {
            ...
            [self cjg_setArgumentWithObject:arg atIndex:0];
        }else{
            for (NSUInteger i = 0; i < numargs; i++) {
                [self cjg_setArgumentWithObject:array[i] atIndex:i];
            }
        }
    }
}
