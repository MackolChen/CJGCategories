...
- (NSData *)cjg_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self cjg_isGzippedData])
    {
        return self;
    }
    
    void *libz = cjg_libzOpen();
...
- (NSData *)cjg_gzippedData
{
    return [self cjg_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)cjg_gunzippedData
{
    if (self.length == 0 || ![self cjg_isGzippedData])
    {
        return self;
